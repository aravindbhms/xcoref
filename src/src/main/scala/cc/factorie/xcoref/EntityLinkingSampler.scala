...
class LinkingSamplerRankTrainer(linker: LinkTrainingSampler, optimizer: GradientOptimizer) extends SampleRankTrainer[Null](linker, optimizer) {
...
class KBEntitySampler(model: Model) extends HierCorefSampler[KBEntity](model) with SamplingStatistics with CoreferenceInferencer[KBEntity] {
  ...
  def sampleCanopyName(context: KBEntity): String = {
    if (context.bagOfMentions.size == 0) {
      println(context.canonicalName)
    }

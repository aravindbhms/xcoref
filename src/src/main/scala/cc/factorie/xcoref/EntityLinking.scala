...
object EntityLinking {
  ...
  def trainAndTest(): Unit = {
    ...
    //
    //testing parameters
    val testSteps = 1000000
    val includeSupplementalMentionsForTesting = true
...
object Baselines {
  ...
  def main(args: Array[String]) {
    ...
    println("ref# " + refKB.entities.size)
    println("Loading TACKBP documents")
    val docs = DEFTJson.deserializeFile(docsFile).take(10).toIndexedSeq
    val docEntities = docs.flatMap(_.attr[DocEntities]).filter(_.useForCoreference).toSeq
    val wpEntities = Loader.deserializeWikipediaEntities(wpFile).take(10).toSeq
    var progress = 0
